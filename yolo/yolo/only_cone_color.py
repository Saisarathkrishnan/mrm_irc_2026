...
class YOLOConeNode(Node):
    ...
    def process(self):
        ...
        for d in out.T:
            ...
            dets.append({"box": (x1,y1,x2,y2), "conf": float(d[4]), "cx": cx, "dist": dist})
        ...
        for d in dets:
            ...
            box_col = BOX_COLOR[color]

            cv2.rectangle(frame, (x1,y1), (x2,y2), box_col, 2)
            cv2.putText(frame, color, (x1, max(0,y1-6)),
            ...
            msg.x = d["dist"]
            msg.y = (d["cx"] - W*0.5) * d["dist"] / FX if not np.isnan(d["dist"]) else 0.0
