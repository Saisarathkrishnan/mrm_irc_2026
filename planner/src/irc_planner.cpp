...
void SensorCallback::stateCallback(const std_msgs::msg::Bool::SharedPtr state)
{
    ...
    if (!rover_state)
    {
        ...
        pickup_cone_location_valid = false;
    ...
    // RESET pickup cone GPS when entering autonomy
    pickup_cone_location_valid = false;
...
void SensorCallback::navigationModeSelect()
{
    ...
    else if (nav_mode == 1)
    {
        ...
        // SAVE PICKUP CONE GPS ONCE
        if (!pickup_cone_location_valid)
        {
            pickup_cone_location = curr_location;
            pickup_cone_location_valid = true;

            RCLCPP_INFO(get_logger(),"[PICKUP] Cone location locked | lat=%.6f lon=%.6f",pickup_cone_location.latitude,pickup_cone_location.longitude);
        }
...
void SensorCallback::setGoalStatus()
{
    ...
    if (isNearPickupConeLocation())
    {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,"[GOAL] Pickup cone detected again → returning to SEARCH");
        cone_detect = false;
        cone_goal_reached = false;
        timing_active = false;
...
bool SensorCallback::isNearPickupConeLocation(double radius_m)
{
    if (!pickup_cone_location_valid)
        return false;

    return haversine(curr_location, pickup_cone_location) < radius_m;
}
